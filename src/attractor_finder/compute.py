...
def compute_attractor_single_thread(
    coeffs, render_iterates, dimension, render_check_ratio=0.01
):
...
def compute_attractor(
    coeffs, render_iterates, dimension, render_check_ratio=0.01, n_processes=6
):
    ...
    args_list = [
        (thread_iterates, coeffs, x0_pool[i], dimension) for i in range(n_processes)
    ]
